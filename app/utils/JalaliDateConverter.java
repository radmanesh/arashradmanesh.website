...
public class JalaliDateConverter {
	...
	private static final List<String> monthNames = Arrays.asList("فروردین","اردیبهشت","خرداد","تیر","مرداد","شهریور","مهر","آبان","آذر","دی","بهمن","اسفند");
	...
	public static String formattedPersian(Date date) {
		...
		String persianDate = persianValueOf(day) + " " + monthNames.get(month-1) + " " + persianValueOf(year);
		return persianDate;
	}
	
	private static String persianValueOf(int n){
		...
		while(k>0){
			int decimal = k % 10;
			k = k / 10;
			sb.insert(0, persianChars[decimal]);
		}
